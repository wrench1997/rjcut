      ...
      runningTasks.forEach((task) => {
        ...
        ;(async () => {
          try {
            ...
            const fileName = task.fileName || `dh_${task.personName || 'human'}_${Date.now()}_${task.id.slice(-6)}.mp4`
      ...
      await Promise.all(initialTasks.map(async (task, index) => {
        ...
        try {
          ...
          const taskPayload = {
            ...
            figure_type: advancedSettings.figure_type || 'whole_body',
          ...
          const fileName = `dh_${selectedPerson.name}_${Date.now()}_${index + 1}.mp4`
