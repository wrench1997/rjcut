...
_CUSTOM_PROMPT_BLOCK_RULES = (
    ...
    (
        "fraud_instruction",
        r"(?:编造|伪造|冒充|虚构|骗过|规避平台|躲审核).{0,18}(?:资质|案例|销量|检测|审核|平台|用户|功效)?",
        "自定义提示词疑似要求虚构或规避审核",
    ),
)
...
def _find_regex_violations(text: str, rules) -> List[Dict[str, str]]:
    import re
    value = str(text or "")
    hits = []
...
def _validate_custom_prompt(custom_prompt: str, max_length: int = 1200) -> None:
    value = str(custom_prompt or "").strip()
    if len(value) > max_length:
        raise ValueError(f"自定义提示词过长，最多允许 {max_length} 个字符")
    hits = _find_regex_violations(value, _CUSTOM_PROMPT_BLOCK_RULES)
    if hits:
        detail = "；".join(item["message"] for item in hits)
...
def _strip_director_words(text: str) -> str:
    import re
    value = str(text or "")
    value = re.sub(r"(?:转场|切镜|镜头切到|画面切到|画面给到|这里放素材|插入素材)", "", value, flags=re.I)
...
def _normalize_copywriting_script(raw: dict, template_structure: List[Dict]) -> dict:
    ...
    template_structure = template_structure if isinstance(template_structure, list) else []
    ...
    for index in range(max_len):
        generated = raw_segments[index] if index < len(raw_segments) and isinstance(raw_segments[index], dict) else {}
        template_seg = template_structure[index] if index < len(template_structure) and isinstance(template_structure[index], dict) else {}
        text = _strip_director_words(generated.get("text") or "")
        if not text:
            continue

        source_flag = str(template_seg.get("flag") or generated.get("visual_mode") or generated.get("flag") or "auto").lower()
        if source_flag in {"scene", "transition"}:
            visual_mode = "scene"
            scene_index += 1
            slot_id = generated.get("slot_id") or template_seg.get("slot_id") or f"slot_{scene_index}"
        ...
        visual_tags = generated.get("visual_tags") if isinstance(generated.get("visual_tags"), list) else []
        ...
        normalized_segments.append({
            ...
            "slot_id": slot_id,
            "flag": "scene" if visual_mode == "scene" else ("hook" if index == 0 else "ending" if index == max_len - 1 else "human"),
            "note": str(generated.get("note") or template_seg.get("note") or ""),
        })

    spoken_text = _strip_director_words(raw.get("spoken_text") or "".join(seg["text"] for seg in normalized_segments))
    if not spoken_text:
        raise ValueError("AI 返回的 spoken_text 为空")

    # 确保段落文字按顺序能在 spoken_text 中找到；不一致时使用段落顺序重建原文。
    cursor = 0
    aligned = True
    for segment in normalized_segments:
        pos = spoken_text.find(segment["text"], cursor)
        if pos < 0:
            aligned = False
            break
        cursor = pos + len(segment["text"])
    if not aligned:
        spoken_text = "".join(segment["text"] for segment in normalized_segments)
...
async def ai_generate_script_via_gateway(
    ...
    template_json = json.dumps(template_structure or [], ensure_ascii=False)
...
5. 全文适合直接发送给数字人 API。"""
    ...
    try:
        async with httpx.AsyncClient(timeout=160.0) as client:
            ...
            script = _normalize_copywriting_script(parsed, template_structure)
            return {
                "success": True,
                "script": script,
