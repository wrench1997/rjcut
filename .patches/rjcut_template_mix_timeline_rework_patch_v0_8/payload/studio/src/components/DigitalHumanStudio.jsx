...
function BatchScriptInput({ scripts, setScripts, onAIGenerate }) {
  ...
  const handleRemove = (id) => setScripts(scripts.filter((script) => script.id !== id))

  const handleChange = (id, text) => {
    setScripts(scripts.map((script) => {
      if (script.id !== id) return script
      if (!script.copywritingPlan) return { ...script, text }
      return {
        ...script,
        text,
        copywritingPlan: {
          ...script.copywritingPlan,
          spoken_text: text,
          meta: {
            ...(script.copywritingPlan.meta || {}),
            needs_segment_review: true,
          },
        },
      }
    }))
  ...
  const handleSegmentMode = (scriptId, segmentId, visualMode) => {
    setScripts((current) => current.map((script) => {
      if (script.id !== scriptId || !script.copywritingPlan) return script
      let sceneIndex = 0
      const segments = script.copywritingPlan.segments.map((segment) => {
        const nextMode = segment.id === segmentId ? visualMode : segment.visual_mode
        if (nextMode === 'scene') sceneIndex += 1
        return {
          ...segment,
          visual_mode: nextMode,
          slot_id: nextMode === 'scene'
            ? (segment.slot_id || `slot_${sceneIndex}`)
            : null,
          edit_action: nextMode === 'scene' ? 'replace_visual' : 'keep_digital_human',
          is_transition_segment: nextMode === 'scene',
          transition: {
            ...(segment.transition || {}),
            enabled: nextMode === 'scene',
            action: nextMode === 'scene' ? 'replace_visual' : 'keep_digital_human',
            slot_id: nextMode === 'scene'
              ? (segment.slot_id || `slot_${sceneIndex}`)
              : null,
      ...
      const normalized = normalizeCopywritingPlan({
        ...script.copywritingPlan,
        spoken_text: script.text,
        segments,
      })
      return {
        ...script,
        copywritingPlan: normalized,
        note: `AI 结构化文案（${normalized.segments.length} 个语义段，${normalized.transition_segments.length} 个场景替换段）`,
      }
    }))
  }
    ...
    <>
      <div className="p-4 border-b border-slate-200 bg-white flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">AI 文案会同时保存纯口播与场景替换段</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {scripts.map((script, idx) => {
          const plan = script.copywritingPlan
          ...
          return (
            <div key={script.id} className="bg-white rounded-lg shadow-sm border border-slate-200 p-3">
              ...
              <textarea
                className="w-full h-24 text-sm resize-none border-0 bg-transparent focus:outline-none focus:ring-0"
                placeholder="输入该视频的播报文案..."
                value={script.text}
                onChange={(event) => handleChange(script.id, event.target.value)}
              />
              ...
              {plan?.segments?.length > 0 && (
                <div className="mt-3 border-t border-slate-100 pt-3">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <p className="text-xs font-semibold text-slate-700">剪辑段落</p>
                      <p className="text-[10px] text-slate-400">
                        {transitionCount} 段替换场景；生成数字人后按 char_timings 写入精确毫秒时间
                      </p>
                    </div>
                    <span className="text-[10px] rounded-full bg-purple-50 text-purple-700 px-2 py-1">
                      JSON 结构化保存
                    </span>
                  </div>
                    ...
                    {plan.segments.map((segment, segmentIndex) => {
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-lg border p-2 ${isScene ? 'border-purple-200 bg-purple-50' : 'border-slate-200 bg-slate-50'}`}
                        >
                          <div className="flex items-center justify-between gap-2 mb-1.5">
                            <div className="flex items-center gap-2 min-w-0">
                              <span className="text-[10px] font-mono text-slate-400">
                                {segment.id || `s${segmentIndex + 1}`}
                              </span>
                              <span className={`text-[10px] rounded px-1.5 py-0.5 ${isScene ? 'bg-purple-600 text-white' : 'bg-blue-600 text-white'}`}>
                                {isScene ? '场景替换' : '数字人'}
                              </span>
                              {isScene && (
                                <span className="text-[10px] text-purple-700 truncate">
                                  {segment.slot_id || `slot_${segmentIndex + 1}`}
                                </span>
                              )}
                            </div>
                            <div className="flex rounded border border-slate-200 overflow-hidden shrink-0">
                              <button
                                type="button"
                                onClick={() => handleSegmentMode(script.id, segment.id, 'human')}
                                className={`px-2 py-1 text-[10px] ${!isScene ? 'bg-blue-600 text-white' : 'bg-white text-slate-500'}`}
                              >
                                数字人
                              </button>
                              <button
                                type="button"
                                onClick={() => handleSegmentMode(script.id, segment.id, 'scene')}
                                className={`px-2 py-1 text-[10px] ${isScene ? 'bg-purple-600 text-white' : 'bg-white text-slate-500'}`}
                              >
                                场景
                              </button>
                            </div>
                          </div>
                          <p className="text-[11px] leading-5 text-slate-700">{segment.text}</p>
                          {segment.visual_tags?.length > 0 && (
                            <p className="text-[10px] text-slate-400 mt-1">
                              素材标签：{segment.visual_tags.join('、')}
                            </p>
                          )}
                        </div>
                      )
