...
export const DEFAULT_TEMPLATES = [
  ...
  },
  {
    ...
    segments: [
      { flag: 'hook', note: '开场吸引 - 数字人出镜' },
      { flag: 'scene', note: '产品介绍素材' },
      { flag: 'scene', note: '成分展示素材' },
      { flag: 'scene', note: '使用演示素材' },
      { flag: 'scene', note: '使用反馈素材' },
      { flag: 'ending', note: '结尾引导 - 数字人出镜收尾' },
    ],
    style: {
  ...
  },
  {
    ...
    segments: [
      { flag: 'hook', note: '开场吸引 - 数字人出镜' },
      { flag: 'scene', note: '产品展示素材' },
      { flag: 'scene', note: '优惠信息素材' },
      { flag: 'ending', note: '结尾引导 - 数字人出镜收尾' },
    ],
    style: {
  ...
  },
  {
    ...
    segments: [
      { flag: 'hook', note: '开场吸引 - 数字人出镜' },
      { flag: 'scene', note: '品质展示素材' },
      { flag: 'scene', note: '使用场景素材' },
      { flag: 'ending', note: '结尾引导 - 数字人出镜收尾' },
    ],
    style: {
...
export function sanitizeCopywritingTemplateStructure(segments = []) {
  ...
  return (Array.isArray(segments) ? segments : []).map((segment, index) => {
    ...
    return {
      ...
      text: directorWords.test(rawText) ? '' : rawText,
      visual_mode: isTransitionSegment ? 'scene' : (['hook', 'human', 'ending'].includes(flag) ? 'human' : flag),
      slot_id: slotId,
      edit_action: isTransitionSegment ? 'replace_visual' : 'keep_digital_human',
      is_transition_segment: isTransitionSegment,
      transition: {
        enabled: isTransitionSegment,
        action: isTransitionSegment ? 'replace_visual' : 'keep_digital_human',
        slot_id: slotId,
...
export async function aiGenerateScript({
  ...
  try {
    ...
    // 鹿场直销风格专用字段
    if (productName) requestBody.product_name = productName
    if (sellingPoints) requestBody.selling_points = sellingPoints
    if (targetAudience) requestBody.target_audience = targetAudience
    if (comparisonProduct) requestBody.comparison_product = comparisonProduct
    if (farmScale) requestBody.farm_scale = farmScale
    if (identificationPoints) requestBody.identification_points = identificationPoints
    if (callToAction) requestBody.call_to_action = callToAction
    ...
    // 新版返回 script={spoken_text,segments,meta}；兼容旧后端的 segments。
    if ((result.code === 0 || result.code === 200) && result.data) {
      if (result.data.script?.spoken_text) return result.data.script
      if (result.data.spoken_text && Array.isArray(result.data.segments)) {
        return {
          schema: 'rjcut.copywriting-plan/v2',
          spoken_text: result.data.spoken_text,
          segments: result.data.segments,
          meta: {},
        }
      }
      if (Array.isArray(result.data.segments)) {
        const spokenText = result.data.segments.map((item) => item.text || '').join('')
        return {
          schema: 'rjcut.copywriting-plan/v2',
 ...
 * @param {number} params.transitionCount - 场景素材段数量（保留参数名兼容后端）
