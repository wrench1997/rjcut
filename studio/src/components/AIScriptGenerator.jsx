...
请直接输出完整口播文案。
请按自然语义分段，禁止在口播中写入导演口令。
不要写镜头说明、不要写标题、不要写分段名称、不要解释创作思路。
全文控制在 180～260 字，优先短句和快节奏。
...
请输出纯口播文案与结构化语义段落，口播中不要出现导演口令，全文控制在 180～260 字。`,
...
- 口语化、接地气
- 文案连贯流畅，段落之间自然衔接
- scene 只表示适合使用环境素材的语义段，文案仍然是自然口播
- 全文控制在 180～260 字

请输出纯口播文案；剪辑位置通过结构化段落表达。`;
};
...
export default function AIScriptGenerator({ onGenerate, loading }) {
  ...
  useEffect(() => {
    if (promptMode !== 'preset') return;
    setCustomPrompt(buildPrompt(selectedTone, '', productInfo));
  }, [selectedTone, productInfo, promptMode]);
  ...
  const handlePresetSelect = (presetKey) => {
    if (PRESET_PROMPTS[presetKey]) {
      ...
      setPromptMode('preset');
      setCustomPrompt(buildPrompt(presetKey, '', productInfo));
      setShowAdvanced(true);
    }
  };
        ...
        <div className="tone-selector">
          {TONE_STYLES.map((style) => (
            <button
              ...
              onClick={() => handleToneSelect(style.value)}
