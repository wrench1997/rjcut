...
PERSON_STATUS_FAILED = 2        # 失败（V2 服务返回 2）
...
def run_dh_generate_video_task(task_id: str, payload: dict, trace_id: str, merchant_id: str):
    ...
    try:
        ...
        if not figure_type:
            ...
            logger.warning(f"payload 中未提供 figure_type，使用默认值：{figure_type}")
        ...
        for _ in range(max_poll_count):
            ...
            if ChanjingStatusCode.is_success(status_resp.get('code')):
                ...
                if status != last_status:
                    logger.info(f"第 {poll_count} 次轮询 - 状态变化：{json.dumps(status_resp, ensure_ascii=False, indent=2)}")
                    last_status = status
                else:
                    logger.info(f"第 {poll_count} 次轮询 - status={status}, progress={progress}, video_url={video_url}, msg={msg}")
                ...
                if status in (VIDEO_STATUS_COMPLETED, VIDEO_STATUS_COMPLETED_ALT):
                    ...
                    logger.error(f"❌ 蝉镜渲染失败：{error_msg}")
                    raise Exception(f"蝉镜渲染失败：{error_msg}")
                elif status == VIDEO_STATUS_PENDING:
                    logger.info(f"⏳ 视频等待中 (status=0)")
                elif status == VIDEO_STATUS_TRANSCODING:
                    logger.info(f"⏳ 视频转码中 (status=3), 进度：{progress}%")
                else:
                    logger.warning(f"⚠️ 未知状态码：status={status}, progress={progress}, msg={msg}")
            else:
                logger.warning(f"❌ 状态查询接口返回异常：code={status_resp.get('code')}, msg={status_resp.get('msg')}")
...
def run_dh_create_person_task(task_id: str, payload: dict, trace_id: str, merchant_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"成功提取视频第一帧：{cover_image_path}")
        except Exception as e:
            logger.warning(f"提取视频封面失败：{e}，将使用蝉镜 API 返回的封面")
        ...
        if cover_image_path and os.path.exists(cover_image_path):
            try:
                ...
                logger.info(f"封面图已上传至 OSS: {cover_oss_key}")
            except Exception as e:
                logger.warning(f"上传封面图到 OSS 失败：{e}")
                cover_oss_key = None
        ...
        for attempt in range(max_retries):
            ...
            if not isinstance(train_resp, dict):
                logger.warning(f"蝉镜训练接口返回格式异常，等待重试：{train_resp}")
            ...
            if train_resp.get('code') == ChanjingStatusCode.SYSTEM_ERROR and '文件还未完成上传' in str(train_resp.get('msg', '')):
                if attempt < max_retries - 1:
                    logger.warning(f"文件未就绪，等待 {retry_delay}s 后重试 ({attempt + 1}/{max_retries})")
                    time.sleep(retry_delay)
                    continue
                else:
        ...
        if isinstance(train_data, str):
            person_id = train_data
        elif isinstance(train_data, dict):
            person_id = train_data.get('id') or train_data.get('person_id')
        ...
        for i in range(480):
            ...
            if ChanjingStatusCode.is_success(status_resp.get('code')):
                ...
                logger.info(f"当前状态码：{status}, 进度：{progress}")
                ...
                if status == PERSON_STATUS_COMPLETED or (status == PERSON_STATUS_COMPLETED_ALT and progress == 100):
                    ...
                    logger.error(f"=== 数字人训练失败详细信息 ===")
                    logger.error(f"person_id: {person_id}")
                    logger.error(f"status: {status}")
                    logger.error(f"progress: {data.get('progress')}")
                    logger.error(f"msg: {data.get('msg')}")
                    logger.error(f"err_reason: {data.get('err_reason')}")
                    logger.error(f"reason: {data.get('reason')}")
                    logger.error(f"trace_id: {status_resp.get('trace_id')}")
                    logger.error(f"完整响应：{json.dumps(status_resp, ensure_ascii=False, indent=2)}")
                ...
                logger.warning(f"状态查询接口返回异常：{json.dumps(status_resp, ensure_ascii=False)}")
            time.sleep(30)
